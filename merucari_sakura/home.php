...
try {
 ...
 exit('DbConnectError:'.$e->getMessage());
			...
			// SQL文(売りたいリスト)
				...
				$stmt = $pdo->prepare("SELECT * FROM marc_table INNER JOIN shape ON marc_table.shape = shape.shape_id INNER JOIN category ON marc_table.category = category.category_id");
				$status = $stmt->execute();
				$view ="";
				if($status==false){
					$error = $stmt->errorInfo();
					...
					while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
					  ...
					  $view .='<a href="mypage_change.php?marc_num='.$row["marc_num"].'">[購入]</a>';
					  $view .='</div>';					 
